...
const sectionToAnswerSheetService = {
  getSectionToAnswerSheetWithAnswers: async (section2ASId: string) => {
    let config = {
      headers: {
  ...
  getOneSectionToAnswerSheet: async (section2ASId: number) => {
    let config = {
      headers: {
        Authorization: `Bearer ${cookie.load('token')}`,
      },
    ...
    try {
      const response = await axios.get(
        `${API_URL}/section-to-answer-sheet/findOne?key=id&value=${section2ASId}&relations=section`,
        config
      );
      return response;
    } catch (error: any) {
